...
// Select abonnement
$result_commande = selectDB('*', 'commandes_histories', 'email = "' . $result['email'] . '" ORDER BY id DESC', $db, '*');

// Select abonnement
$formules = selectDB('*', 'formules', 'id = "' . $_GET['formule'] . '" ORDER BY id DESC', $db, '1');
...
$commande->set_var($result_commande);

if ($_GET['formule'] == "") {
    header('Location: /gestion-abonnement');
}
...
include 'pages/payment-renewal-accepted.php';
