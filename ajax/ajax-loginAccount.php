...
$login = selectDB('*', 'users', 'email = "' . $email . '" AND password = "' . md5($password) . '"', $db, '1');

if ($login['status'] != 1) {

    if ($email == $login['email']) {

        if ($login['expired_date'] >= date('Y-m-d H:i:s')) {
        ...
        <body class="d-flex flex-column h-100">
            ...
            $body = '
        <div style="text-align: center;padding: 2em 0;">Une connexion a été détectée sur votre compte avec l\'ip suivante :</div><br/>
        <div style="text-align: center;padding: 2em 0;">' . $_SERVER['REMOTE_ADDR'] . '</div>
        ...
        $final = ["logged" => false, "message" => "Votre adresse e-mail ou votre mot de passe sont incorrect !"];
    }
} else {
    $final = ["logged" => false, "message" => "Votre compte a été désactivé par mesure de sécurité."];
}
